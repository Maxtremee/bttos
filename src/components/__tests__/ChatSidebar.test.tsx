...
describe("ChatSidebar", () => {
  it("Test 1: Messages render as a list", () => {
    ...
    dispose = render(
      () => <ChatSidebar messages={msgs} emoteMap={new Map()} status="active" />,
      container,
    );
  ...
  it("Test 4: Status loading-emotes shows Loading chat...", () => {
    ...
    document.body.appendChild(container);
    dispose = render(
      () => <ChatSidebar messages={[]} emoteMap={new Map()} status="loading-emotes" />,
      container,
    );
  ...
  it("Test 5: Status reconnecting shows Reconnecting...", () => {
    ...
    document.body.appendChild(container);
    dispose = render(
      () => <ChatSidebar messages={[]} emoteMap={new Map()} status="reconnecting" />,
      container,
    );
  ...
  it("Test 6: Status active does NOT show status text", () => {
    ...
    document.body.appendChild(container);
    dispose = render(
      () => <ChatSidebar messages={[]} emoteMap={new Map()} status="active" />,
      container,
    );
    const text = container.textContent ?? "";
