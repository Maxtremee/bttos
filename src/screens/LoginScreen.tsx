...
export default function LoginScreen() {
  ...
  return (
    <main class={`${styles.screen} gap-col-xl`}>
      <h1 class={styles.heading}>Sign in with Twitch</h1>
      ...
      <Show when={state() === "polling" || state() === "success"}>
        <div class={`${styles.pollingRow} gap-row-2xl`}>
          ...
          <div class={`${styles.qrColumn} gap-col-md`}>
            <div innerHTML={qrSvg()} class={styles.qrCode} />
            <p class={styles.verificationHost}>{verificationHost()}</p>
          </div>

          {/* Code column */}
          <div id="login-status-region" class={`${styles.codeColumn} gap-col-md`}>
            <p class={styles.instructionText}>
              Go to twitch.tv/activate on your phone or PC, then enter:
            </p>
            <p class={styles.userCode}>{deviceCodeData()?.user_code ?? ""}</p>
            <p class={styles.statusMessage}>{statusMessage()}</p>
          </div>
        </div>
      </Show>
      ...
      <Show when={state() === "expired" || state() === "error"}>
        <div class={`${styles.errorColumn} gap-col-md`}>
          <p class={styles.errorText}>{statusMessage()}</p>
